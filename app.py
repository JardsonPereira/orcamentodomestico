...
# --- NOVO VISUAL: CLEAN GLASSMORPHISM ---
...
# --- AUTENTICAÇÃO ---
def tela_login():
    st.markdown("<h1 style='text-align: center; margin-bottom: 30px;'>🍃 Pocket Finance</h1>", unsafe_allow_html=True)
    ...
    with col:
        with st.container():
            ...
            with aba_up:
                ...
                n_senha = st.text_input("Crie uma senha segura", type="password")
                if st.button("Começar Agora"):
                    try:
                        supabase.auth.sign_up({"email": n_email, "password": n_senha})
                        st.success("Conta criada! Basta fazer o login.")
...
if st.session_state.user is None:
    ...
    st.sidebar.markdown("<h3 style='text-align: center;'>🍃 Pocket Finance</h3>", unsafe_allow_html=True)
    ...
    if menu == "Novo Lançamento":
        ...
        with st.form("form_lan", clear_on_submit=True):
            ...
            desc = st.text_input("Descrição (Ex: Jantar, Aluguel)")
            ...
            cartao_sel = st.selectbox("Forma de Pagamento", ["Dinheiro/Pix/Débito"] + lista_c)
            parcelas = st.number_input("Dividir em quantas vezes?", min_value=1, value=1)
    ...
    # --- ABA: DASHBOARD MENSAL ---
        ...
        if res.data:
            ...
            c3.metric("Sobrou", format_real(rec - des))
            ...
            st.info("Aguardando seu primeiro lançamento.")

    # --- ABA: CARTÕES DE CRÉDITO ---
    elif menu == "Cartões de Crédito":
        st.markdown("## 💳 Seus Cartões")
        tab1, tab2, tab3 = st.tabs(["📅 Fatura Atual", "📋 Compras Totais", "🛠️ Gerenciar"])
            ...
            if not df_all.empty:
                ...
                for cartao in df_f['cartao_nome'].unique():
                    with st.expander(f"Fatura {cartao}", expanded=True):
                        ...
                        st.markdown(f"**Total a pagar:** <span style='color: #6366f1; font-size: 1.2rem;'>{format_real(df_c['valor'].sum())}</span>", unsafe_allow_html=True)
            ...
            if not df_all.empty:
                ...
                for (desc, cartao), df_compra in grupos:
                    with st.container():
                        ...
                        val_total = df_compra['valor'].sum()
                        data_u = df_compra['data'].max().strftime('%d/%m/%Y')
                        saldo_r = df_compra[df_compra['data'].dt.date >= date.today()]['valor'].sum()

                        c1.markdown(f"**{desc}**<br><small>{cartao}</small>", unsafe_allow_html=True)
                        c2.write(f"Total: **{format_real(val_total)}**")
                        c3.write(f"A pagar: **{format_real(saldo_r)}**")
                        c4.write(f"Fim: {data_u}")
